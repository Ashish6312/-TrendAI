...
def get_recommendations(request: RecommendationRequest, db: Session = Depends(get_db)):
    ...
    try:
        print("🔄 Calling generate_dynamic_recommendations...")
...
def get_business_plan(request: BusinessPlanRequest):
    ...
    print(f"🚀 Generating business plan for: {request.business_title} in {request.area}")
    ...
    try:
        ...
        print(f"✅ Generated business plan for {request.business_title}")
        return business_plan
        
    except Exception as e:
        print(f"❌ Error generating business plan: {e}")
...
def get_roadmap(request: RoadmapRequest):
    ...
    try:
        ...
        print(f"✅ Generated roadmap for {request.title}")
        return roadmap_data
...
def get_user_profile(email: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "statistics": {
            ...
            "account_age_days": (datetime.now(user.created_at.tzinfo) - user.created_at).days if user.created_at else 0
        },
        "subscription": {
            "plan_name": subscription.plan_name if subscription else "free",
            "plan_display_name": subscription.plan_display_name if subscription else "Free Plan",
            ...
            "max_analyses": subscription.max_analyses if subscription else 3
