...
export function SubscriptionProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load subscription plan from localStorage, API, or payment detection
  useEffect(() => {
    const loadUserPlan = async () => {
      if (session?.user?.email) {
        try {
          // First check localStorage
          const savedPlan = localStorage.getItem(`subscription_${session.user.email}`) as SubscriptionPlan;
          if (savedPlan && ['free', 'professional', 'enterprise'].includes(savedPlan)) {
            setPlanState(savedPlan);
          }

          // Then check if there's a recent payment (from URL params or session storage)
          ...
          if (paymentPlan) {
            ...
            const newPlan = planMapping[paymentPlan] || 'professional';
            setPlan(newPlan);
          }

          // Check session storage for recent payment
          const recentPayment = sessionStorage.getItem('recent_payment_plan');
          if (recentPayment) {
            const planMapping: Record<string, SubscriptionPlan> = {
              'Market Explorer': 'free',
              'Growth Accelerator': 'professional', 
            ...
            const newPlan = planMapping[recentPayment] || 'professional';
            setPlan(newPlan);
            sessionStorage.removeItem('recent_payment_plan'); // Clear after use
          }
