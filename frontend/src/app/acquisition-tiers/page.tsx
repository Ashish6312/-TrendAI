...
export default function AcquisitionTiers() {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] relative selection:bg-emerald-500/30 overflow-x-hidden">
      {/* Immersive Background Architecture */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-emerald-500/5 rounded-full blur-[120px] animate-pulse-slow" />
        <div className="absolute bottom-1/4 right-0 w-[600px] h-[600px] bg-purple-600/5 rounded-full blur-[150px] animate-pulse-slow" style={{ animationDelay: '2s' }} />
      ...
      <div className="relative z-10">
          ...
          >
            <div className="flex flex-col items-center gap-8">
              <motion.div 
                ...
                className="inline-flex items-center gap-4 px-6 py-3 rounded-full bg-white/5 border border-white/10 backdrop-blur-xl shadow-2xl"
              >
              ...
              <h1 className="responsive-text-5xl md:responsive-text-6xl lg:text-7xl font-black text-white tracking-tighter leading-tight italic">
                Master the <br />
                <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-500 animate-gradient-x py-2 inline-block">Pricing Plans.</span>
              </h1>
 
              <p className="responsive-text-base md:responsive-text-lg text-slate-400 max-w-2xl mx-auto font-medium leading-relaxed tracking-tight px-4 opacity-60">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-0 max-w-5xl mx-auto w-full items-center divide-y md:divide-y-0 md:divide-x divide-white/5">
              {[
                { label: "Capital Deployed", value: "$2.3B+", color: "text-emerald-400", sub: "Global Institutional Flow" },
                { label: "Market Advantage", value: "94%", color: "text-blue-400", sub: "Alpha Detection Accuracy" },
                { label: "Precision Rate", value: "99.9%", color: "text-purple-400", sub: "Operational Reliability" }
              ].map((stat, i) => (
                <div key={i} className="px-12 py-8 md:py-0 text-center group cursor-default transition-all">
                  ...
                  <div className="text-[10px] text-slate-200 font-black uppercase tracking-[0.3em] italic">{stat.label}</div>
                  <div className="text-[8px] text-slate-600 font-bold uppercase tracking-[0.2em] mt-2">{stat.sub}</div>
                </div>
              ))}
            </div>
            ...
            <div className="relative flex flex-col items-center gap-24 pt-10">
              ...
              {userLocation?.country && (
                <motion.div 
                  ...
                  className="flex items-center gap-5 text-[11px] font-black uppercase tracking-[0.5em] text-slate-400 bg-white/[0.02] px-12 py-6 rounded-3xl border border-white/5 italic mt-16 mb-16 shadow-4xl backdrop-blur-3xl"
                >
                  <div className="p-2.5 rounded-xl bg-blue-500/10 text-blue-500">
                    <Globe size={18} className="animate-pulse" /> 
                  </div>
                  <span className="flex items-center gap-3">
                    Market Region: <span className="text-slate-200">{userLocation.country}</span> Pricing Selected
                  </span>
                </motion.div>
              )}
            </div>
        ...
        <div className="responsive-container pb-40 pt-40 border-t border-white/5 mt-40 relative overflow-visible">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--background)] px-12 text-[11px] font-black uppercase tracking-[0.6em] text-slate-800 italic border-x border-white/5">
            Deployment Matrix
          </div>
            ...
            {tiers.map((tier, index) => (
              <motion.div
                ...
                className={`group relative flex flex-col rounded-[2.5rem] p-10 md:p-12 h-full backdrop-blur-3xl border transition-all duration-700 ${
                  tier.popular 
                    ? 'bg-gradient-to-b from-emerald-500/10 via-slate-900/90 to-slate-900/95 border-emerald-500/30 shadow-[0_40px_100px_-20px_rgba(16,185,129,0.2)] scale-105 z-20' 
                    : tier.id === 'enterprise' 
                    ? 'bg-gradient-to-b from-purple-500/10 via-slate-900/90 to-slate-900/95 border-purple-500/20 hover:border-purple-500/40' 
                ...
                <div className="flex-1 space-y-10 flex flex-col relative z-20">
                  ...
                  <div className="text-center space-y-8">
                    ...
                    <div className="space-y-3">
                      <h3 className="text-3xl md:text-4xl font-black text-white tracking-tighter leading-none italic">{tier.name}</h3>
                      <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] h-4">{tier.tagline}</p>
                    </div>
                  </div>

                  {/* Pricing Display - Animated */}
                  <div className="flex flex-col items-center justify-center min-h-[140px] bg-white/[0.02] rounded-3xl p-8 border border-white/5 relative group/price overflow-hidden">
                    ...
                    {tier.originalPrice && (
                      <div className="text-slate-600 text-xs font-bold line-through mb-2 opacity-40">
                    ...
                    <div className="flex flex-col items-center gap-1">
                      <div className="flex items-baseline gap-2">
                        ...
                        {typeof tier.price === 'number' && tier.price > 0 && (
                          <span className="text-slate-400 font-extrabold text-[10px] uppercase tracking-widest">/mo</span>
                        )}
                      </div>
                      {billingCycle === 'yearly' && typeof tier.price === 'number' && tier.price > 0 && (
                        <div className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Billed Annually</div>
                      )}
                    </div>
                    <div className="text-emerald-500 text-[9px] font-black uppercase tracking-widest mt-4 flex items-center gap-2">
                  ...
                  <div className="space-y-5 pt-8 mt-auto px-2">
                    {tier.features.map((feature, idx) => (
                      <div key={idx} className="flex items-start gap-4 group/item">
                        ...
                        <span className={`text-[11px] font-bold tracking-tight transition-all ${feature.available ? 'text-slate-300 group-hover/item:text-white' : 'text-slate-700 line-through opacity-40'}`}>
                          {feature.text}
                        </span>
                      </div>
                    ))}
                  ...
                  {/* Trust Footer */}
                  <div className="pt-10 flex items-center justify-center gap-2 opacity-30 group-hover:opacity-100 transition-opacity">
                     <Shield size={12} className="text-emerald-500" />
                     <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Secured via Enterprise SSL</span>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <div className="bg-slate-900/40 border-y border-white/5 py-32 backdrop-blur-3xl relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center mb-24 space-y-4">
            <div className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-500 italic">Competitive Edge</div>
            <h2 className="text-4xl md:text-6xl font-black text-white italic tracking-tighter">Why Settle for Average?</h2>
            <p className="text-slate-500 font-medium max-w-xl mx-auto">Deploy the most advanced market acquisition tools in the industry.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="group p-10 rounded-[2.5rem] bg-white/[0.02] border border-white/5 hover:bg-white/[0.04] hover:border-white/10 transition-all duration-500"
              >
                <div className={`mb-8 inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-${benefit.color}-500/10 text-${benefit.color}-400 group-hover:scale-110 group-hover:rotate-6 transition-all`}>
                  {benefit.icon}
                </div>
                <h3 className="text-2xl font-black text-white italic tracking-tighter mb-4">{benefit.title}</h3>
                <p className="text-slate-400 text-sm leading-relaxed font-medium opacity-80">{benefit.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <div className="py-32 relative">
        <div className="max-w-4xl mx-auto px-6">
          <div className="text-center mb-24 space-y-4">
            <div className="text-[10px] font-black uppercase tracking-[0.4em] text-purple-500 italic">Common Questions</div>
            <h2 className="text-4xl md:text-5xl font-black text-white italic tracking-tighter">Clarify Your Path</h2>
          </div>
          ...
          <div className="space-y-4">
            {[
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="group bg-white/[0.01] hover:bg-white/[0.03] rounded-3xl p-8 border border-white/5 hover:border-white/10 transition-all duration-300"
              >
                <h3 className="text-lg font-black text-white italic tracking-tighter mb-3 group-hover:text-emerald-400 transition-colors flex items-center justify-between">
                  {faq.question}
                  <ArrowRight size={16} className="opacity-0 group-hover:opacity-100 transition-all -translate-x-4 group-hover:translate-x-0" />
                </h3>
                <p className="text-slate-400 text-sm font-medium leading-relaxed opacity-70">{faq.answer}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
          ...
          >
            <div className="space-y-6">
              <h2 className="text-5xl md:text-7xl font-black text-white italic tracking-tighter leading-none">Execute Your <br /> Vision Now.</h2>
            ...
            <button
              ...
              className="group relative inline-flex items-center gap-6 px-12 py-6 bg-white text-slate-950 font-black text-xs uppercase tracking-[0.4em] rounded-[2rem] hover:scale-110 active:scale-95 transition-all shadow-[0_20px_60px_rgba(255,255,255,0.2)]"
            >
            ...
            <div className="pt-8 flex items-center justify-center gap-3 text-[9px] font-black uppercase tracking-[0.2em] text-slate-600 italic">
