      ...
      <AnimatePresence mode="wait">
        {activeView === 'overview' && (
          ...
          >
              ...
              <div className="lg:col-span-2">
                ...
                >
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 rounded-xl p-4 border border-green-500/20">
                      <p className="text-green-600 dark:text-green-400 text-sm font-medium">Investment Required</p>
                      <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">
                        {businessData.business.funding_required || '₹5L-₹15L'}
                      </p>
                    </div>
                    <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 rounded-xl p-4 border border-blue-500/20">
                      <p className="text-blue-600 dark:text-blue-400 text-sm font-medium">Expected Revenue</p>
                      <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">
                        {businessData.business.estimated_revenue || '₹25L/year'}
                      </p>
                    </div>
                    <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-xl p-4 border border-purple-500/20">
                      <p className="text-purple-600 dark:text-purple-400 text-sm font-medium">ROI</p>
                      <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">
                        {businessData.business.roi_percentage || '120'}%
                      </p>
                    </div>
                    <div className="bg-gradient-to-r from-orange-500/10 to-red-500/10 rounded-xl p-4 border border-orange-500/20">
                      <p className="text-orange-600 dark:text-orange-400 text-sm font-medium">Payback Period</p>
                      <p className="text-2xl font-black text-slate-900 dark:text-white mt-1">
            ...
            {contactInfo && (
              <UniformCard 
                title="Business Support & Networking" 
                subtitle="Local resources and contacts to help you get started"
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Business Associations</h4>
                    <div className="space-y-3">
                      {contactInfo.businessAssociations.map((association: any, index: number) => (
                        <div key={index} className="bg-slate-50 dark:bg-white/5 rounded-lg p-4 border border-slate-200 dark:border-white/10">
                          <h5 className="text-slate-900 dark:text-white font-semibold mb-2">{association.name}</h5>
                          <div className="space-y-1">
                            <div className="flex items-center space-x-2">
                              <Phone className="w-3 h-3 text-slate-400" />
                              <span className="text-xs text-slate-600 dark:text-gray-400">{association.phone}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <Mail className="w-3 h-3 text-slate-400" />
                              <span className="text-xs text-slate-600 dark:text-gray-400">{association.email}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  {/* Local Support */}
                  <div>
                    <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Local Support Services</h4>
                    <div className="space-y-3">
                      {contactInfo.localSupport.map((support: any, index: number) => (
                        <div key={index} className="bg-slate-50 dark:bg-white/5 rounded-lg p-4 border border-slate-200 dark:border-white/10">
                          <h5 className="text-slate-900 dark:text-white font-semibold mb-2">{support.name}</h5>
                          <div className="space-y-1 mb-3">
                            <div className="flex items-center space-x-2">
                              <Phone className="w-3 h-3 text-slate-400" />
                              <span className="text-xs text-slate-600 dark:text-gray-400">{support.phone}</span>
                            </div>
                            <div className="flex items-center space-x-2">
                              <Mail className="w-3 h-3 text-slate-400" />
                              <span className="text-xs text-slate-600 dark:text-gray-400">{support.email}</span>
                            </div>
                          </div>
                          {support.services && (
                            <div className="flex flex-wrap gap-1">
                              {support.services.slice(0, 3).map((service: string, serviceIndex: number) => (
                                <span key={serviceIndex} className="px-2 py-1 bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded text-xs font-medium">
                                  {service}
                                </span>
                              ))}
                            </div>
